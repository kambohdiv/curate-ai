...
export default function About({
  ...
  // Handle the "Hire Me" button clicks
  const handleHireMeClick = () => {
    if (clickTimeout) {
      clearTimeout(clickTimeout); // Cancel the timeout if it's a double-click
      setClickTimeout(null);
      handleDoubleClick(); // Trigger double-click redirection
    } else {
      const timeout = setTimeout(() => {
        setClickTimeout(null);
        openEmailModal(); // Open modal on single-click
  ...
  }, [profileImage, userEmail, emailBody, emailSubject, userURL]);
  ...
  return (
    <div className="md:h-[408.01px] flex flex-col justify-center gap-7 sm:px-8 p-8 bg-[#1b1b1b] mt-5 border-2 border-neutral-800 rounded-xl ">
      <div className="flex md:flex-row  flex-col-reverse  justify-between md:items-center items-start md:gap-0 gap-7  ">
        <h4 className='text-2xl font-medium text-[#c0c0c0] relative title' contentEditable>UI/UX Designer</h4>
        <div className="bg-[#171717]  border border-[#3f3e3e]  px-6 py-2 gap-3 rounded-full flex justify-center items-center ">
          ...
          <span className='md:text-lg text-sm status' contentEditable >AVAILABLE FOR JOB</span>
        </div>
      </div>
      <div className="flex md:flex-row  flex-col-reverse  justify-between md:items-center items-start gap-6">
        <div className="flex flex-col  relative  justify-center items-start gap-2">
          <h1 className='text-white md:text-4xl text-3xl font-semibold name' contentEditable >I am Zainab Nisa</h1>
          <p className='text-[#c0c0c0] text-lg font-normal description' contentEditable>I design digital products with great experience.</p>
        ...
        <div className="w-[178px] h-[178px] bg-[#2c2c2c] flex justify-center items-center rounded-full border mr-5 border-[#3f3e3e]">
          <div className="w-[150px] h-[150px] bg-[#2c2c2c] relative rounded-full border border-[#3f3e3e]">
            <img src={profileImage} alt="profile" className="rounded-full w-[150px] h-[150px]" />
            <label htmlFor="uploadProfile" className="text-white cursor-pointer"> 
              <div className="w-[150px] h-[150px] absolute inset-0  z-50 rounded-full bg-black/20 flex justify-center items-center">
                <input type="file" id="uploadProfile" className="hidden" onChange={handleImageChange} />
              </div>
            </label>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-30 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-[#1b1b1b] border-2 border-neutral-800 p-6 rounded-lg">
            ...
            <div className="flex justify-end gap-2">
              <button onClick={closeModal} className="px-4 py-2 bg-gray-600 text-white rounded-md">Cancel</button>
              <button onClick={handleFormSubmit} className="px-4 py-2 bg-[#e63e21] text-white rounded-md">Add</button>
            </div>
          </div>
        </div>
      )}
